...
Our exploratory data contains detailed data on personal healthcare spending on a per capita basis by state and by service from 1991 to 2020. Here are some ideas related to maternal mortality ratio based on this information:
  ...
  2. Relation to Maternal Mortality Ratio (MMR):
    i: Access to Care: Correlating per capita spending with MMR could reveal if higher spending correlates with lower maternal mortality rates, suggesting that access to care might be a significant factor.
    ii: Funding for Maternal Health: Evaluating whether specific funding (like Medicaid expansions) is associated with improvements in maternal outcomes.
    iii: Rate Idea: Efficiency of Spending: Analyzing if increased spending in certain healthcare services (such as prenatal and postnatal care) is more effective in reducing MMR compared to others.
