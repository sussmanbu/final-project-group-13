...
# Load the dataset (replace 'dataset/phc_mmr_with_age.csv' with the correct path)
data <- read_csv("dataset/phc_mmr_with_age.csv", show_col_types = FALSE)

# Convert AverageAge, spending, and mean_val (representing MMR) to numeric
data$AverageAge <- as.numeric(as.character(data$AverageAge))
data$spending <- as.numeric(as.character(data$spending))
data$mean_val <- as.numeric(as.character(data$mean_val))

# Remove rows with missing or NA values in spending, mean_val (MMR), or AverageAge
data <- data[!is.na(data$spending) & !is.na(data$AverageAge) & !is.na(data$mean_val), ]

# Create the faceted bubble chart with a fitted line
ggplot(data, aes(x = spending, y =AverageAge,  color = factor(year))) +
  geom_point(alpha = 0.6) +  # Bubble chart
  geom_smooth(method = "lm", se = FALSE, color = "blue", linetype = "dashed") +  # Add the fit line
  scale_size_continuous(range = c(2, 10)) +  # Adjust bubble sizes
  labs(title = "AverageAge vs. Healthcare Spending by Year",
       x = "Spending",
       y = "AverageAge",
       color = "Year") +
