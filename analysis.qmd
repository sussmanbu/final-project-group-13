...
## Motivation
The main purpose of the study is to explore the differences in maternal mortality rates(mmr) among different ethnic groups from 1999 to 2019 across the U.S., and the impact of healthcare spending and average age of pregnancy on maternal mortality rates. By comparing the average maternal mortality rate in each state with other variables,  the result can help to identify the factors that affect maternal mortality rates, thereby enhancing the understanding and protection of women.


## Research Question
1. What are the distributions of maternal mortality rates (MMR) in different regions among different racial groups?
...
### Maternal Mortality by Census Region over Time, Faceted by Race
...
```
• **Regional Trends**:
  - **Midwest Region (Red Line)**: Generally, a rising trend in maternal mortality rates is visible across most racial groups, especially pronounced in Non-Hispanic Black populations.
  
  - **Northeast Region (Green Line)**: This region shows relatively stable or slightly rising rates among all groups, with significant increases visible in the Non-Hispanic Black and Native American groups around 2015.
...
• **Interpretation of Racial Disparities**: Across all regions, **Non-Hispanic Black women** consistently have higher maternal mortality ratios compared to other racial groups, indicating significant racial disparities in maternal healthcare outcomes.

• **Temporal Trends**: A general increase in maternal mortality ratios is observed from 2000 to 2020 across most racial and regional breakdowns, with some fluctuations and sharp increases in specific years and regions. 
...
kable(fit_stats, caption = "Fit Statistics of the Multiple Linear Regression Model")
...
**Effect of Predictors**: 
   ...
   - **AverageAge**: With a coefficient of 2.32 (p < 0.001), a unit increase in average age is associated with an estimated increase of 2.32 units in MMR, holding other factors constant. This implies that as the average age of mothers increases, MMR tends to increase which also fits with precious simple regression result.
...
# 4.Multiplie Linear Regression for MMR-AGE with Healthcare Spending and Race
```{r}
# Load necessary libraries
library(readr)
library(dplyr)
...
# Ensure Age conversion didn't result in unexpected NAs
data <- data[complete.cases(data$AverageAge), ]

# Convert race_group to a factor
data$race_group <- factor(data$race_group)

# Fit the multiple linear regression model
mlr_model <- lm(mean_val ~ spending + race_group, data = data)

# Get the summary of the model and convert it to a data frame
summary_model <- summary(mlr_model)
coef_table <- as.data.frame(summary_model$coefficients)
...
## Flaws and Limitations
  During the examination of the relationship between healthcare spending and maternal mortality rates, it was determined that the correlation between these variables is not robust, suggesting the influence of additional factors on the mortality rates. Furthermore, the data on pregnancy rates in each state lacks detailed ethnic group categorizations, precluding a nuanced correlation analysis that would distinguish the effects of pregnancy rates across different ethnic groups on the maternal mortality rate.
  The healthcare spending data did not consider the inflation rate across the years, which would lead to an obvious increasing trend, and make the later investigation and conclusion less accurate.
...
# Convert race_group to a factor and exclude one level to avoid singularities
data$race_group <- factor(data$race_group)

# Generate dummy variables for race_group, excluding the first level to avoid singularities
race_group_dummies <- model.matrix(~ race_group - 1, data)[, -1]  # -1 to include all levels, excluding intercept

# Combine the val data with the dummy variables
mlr_data <- cbind(data["val"], race_group_dummies)
...
# Display the coefficients in a Kable table
kable(coef_table, caption = "Multiple Linear Regression Coefficients with Race Group")
```
